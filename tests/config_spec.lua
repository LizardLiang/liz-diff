...
describe('liz-diff.config', function()
  ...
  describe('get()', function()
    it('returns default config when no merge has been called', function()
      ...
      assert.are.equal(']f', cfg.keymap.next_file)
      assert.are.equal('[f', cfg.keymap.prev_file)
    end)
  ...
  describe('merge()', function()
    ...
    it('overrides a nav keymap and disables another', function()
      config.merge({ keymap = { next_file = '<Tab>', prev_file = false } })
      assert.are.equal('<Tab>', config.get().keymap.next_file)
      assert.are.equal(false, config.get().keymap.prev_file)
    end)
